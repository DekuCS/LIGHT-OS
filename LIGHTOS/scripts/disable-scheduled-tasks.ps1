...
function Toggle-Task($task, $enable) {
    ...
    $trustedinstaller_task_result = [int](C:\LIGHTOS\MinSudo.exe --NoLogo --TrustedInstaller --Privileged cmd /c "schtasks.exe /change $toggle /tn `"$task`" > nul 2>&1 && echo 0 || echo 1")
...
function main() {
    ...
    Write-Host "LIGHTOS: please wait..."
    ...
    foreach ($wildcard in $wildcards) {
        Write-Host "LIGHTOS: searching for $($wildcard)"
        foreach ($task in $task_names) {
            if ($task.contains($wildcard)) {
                if ((Toggle-Task -task $task -enable $false) -ne 0) {
                    Write-Host "LIGHTOS: error! failed toggling one or more scheduled tasks"
                    return 1
                }
            }
        }
