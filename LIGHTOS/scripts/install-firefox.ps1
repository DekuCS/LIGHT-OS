...
function Fetch-SHA512($source, $file_name) {
    try {
        ...
        Write-Host "LIGHTOS: unable to fetch hash data, consider -skip_hash_check"
        exit 1
    }
...
function main() {
    if (-not (Is-Admin)) {
        Write-Host "LIGHTOS: error! administrator privileges required"
        return 1
    }
    ...
    try {
        ...
        Write-Host "LIGHTOS: error! failed to fetch json data, check internet connection and try again"
        return 1
    }
    ...
    if (Test-Path "$install_dir\firefox.exe" -PathType Leaf) {
        ...
        if ($local_version -eq $remote_version) {
            Write-Host "LIGHTOS: $($folder_name) $($remote_version) already installed"

            if ($force) {
                Write-Host "LIGHTOS: warning! -force specified, proceeding anyway"
            } else {
                return 1
            }
        }
    ...
    Write-Host "LIGHTOS: downloading firefox $remote_version setup"
    ...
    if (-not $skip_hash_check) {
        ...
        if ($local_SHA512 -ne $remote_SHA512) {
            Write-Host "LIGHTOS: error! hash mismatch"
            return 1
        }
    }

    Write-Host "LIGHTOS: installing firefox"
    Stop-Process -Name "firefox" -ErrorAction SilentlyContinue

    try {
        Start-Process -FilePath $setup_file -ArgumentList "/S /MaintenanceService=false" -Wait
    } Catch [System.InvalidOperationException] {
        Write-Host "LIGHTOS: error! failed to download setup file"
        return 1
    }
...
lockPref(`"browser.newtabpage.activity-stream.asrouter.userprefs.cfr.features`", false)"
    ...
    Write-Host "LIGHTOS: release notes: https:/www.mozilla.org/en-US/firefox/$($remote_version)/releasenotes"
